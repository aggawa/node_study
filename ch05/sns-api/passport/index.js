...
module.exports = () => {
   ...
   passport.serializeUser((user, done) => {
      ...
      done(null, user.id) // user테이블의 id값(pk)을 세션에 저장(세션 용량 절약을 위해 id만 저장)
   })

   // 역직렬화(deserializeUser): 클라이언트에게 request가 올때 마다 세션에 저장된 사용자 id(user 테이블의 id컬럼 값)를 바탕으로 사용자 정보를 조회
   passport.deserializeUser((id, done) => {
      // id는 직렬화에서 저장한 user.id값을 가져옴
