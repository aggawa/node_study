...
function PostEditForm({ onPostEdit, initialValues = {} }) {
   ...
   */
   ...
   const [hashtags, setHashtags] = useState(initialValues.Hashtags.map((tag) => `#${tag.title}`).join(' ')) // 해시태그
   ...
   const handleImageChange = (e) => {
      ...
      setImgFile(file) // 업로드한 파일 객체르 state에 저장

      // 이미지 파일 미리보기
      // FileReader() 객체는 파일을 비동기적으로 읽을 수 있도록 도와주는 객체 -> 이미지 미리보기 or 텍스트 파일 읽기 등에 주로 사용한다.
